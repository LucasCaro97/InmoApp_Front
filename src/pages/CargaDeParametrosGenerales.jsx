    ...
    if (token) {
      try {
        const parametrosResponse = await axios.get(
          `${BASE_URL_API}/${tipoParametro.tipoParametro}`,
          {
            headers: {
      ...
      try {
        ...
        if (btnFormValue === "Crear") {
          ...
          const response = await axios.put(
            `${BASE_URL_API}/${tipoParametro.tipoParametro}/${idParametro}`,
            data,
            {
              headers: {
                "Content-Type": "application/json",
        ...
        <form className="flex flex-col gap-8 px-8">
          ...
          <div className="hidden">
            ...
            <input
              ...
              className="w-1/4"
              id="tipoParametro"
              value={tipoParametro.tipoParametro}
              readOnly
            />
          </div>
          ...
          <div>
            <button
              className=" w-full sm:w-40 bg-green-900 text-slate-300 text-lg px-6 py-1 rounded-xl mb-5"
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {parametros.map((parametro) => (
                <tr key={parametro.id}>
                  ...
                  <td>
                    <button
                      className="bg-green-900 text-slate-300 text-lg px-6 py-1 rounded-xl"
                      onClick={() =>
                        handdleEditButton(parametro.id, parametro.nombre)
                      }
                    >
                      {btnValue}
                    </button>
